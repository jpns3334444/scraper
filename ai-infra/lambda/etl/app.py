...
# Add the analysis modules to Python path for import
...
def calculate_ward_medians(properties: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
    """
    Calculate ward-level median prices for scoring.
    ...
    for ward, prices in ward_data.items():
        if len(prices) >= 3:  # Need at least 3 properties for reliable median
