...
def extract_property_images(soup, session, base_url, bucket=None, property_id=None, config=None, date_key=None, logger=None):
    ...
    try:
        ...
        # Apply per-property image cap for performance
...
def main(event=None):
    ...
    # Simple configuration - always use stealth mode with deduplication
    max_properties_limit = config['max_properties'] if config['max_properties'] > 0 else 0  # 0 means no limit
    stealth_enabled = True  # Always use stealth timing
    
    # For local testing, limit to 5 properties for quick feedback
    if is_local_testing and max_properties_limit == 0:
        max_properties_limit = 5
        logger.info("🧪 LOCAL TESTING - Limited to 5 properties for quick feedback")
    
    log_structured_message(logger, "INFO", "Tokyo Real Estate Scraper: Full coverage with stealth and deduplication", 
    ...
    properties_text = f"{max_properties_limit} properties" if max_properties_limit > 0 else "all found properties"
    logger.info(f"🚀 Starting scraper - Session: {config['session_id']}, Target: {properties_text}")
    logger.info(f"🥷 Stealth timing enabled, 🔍 Deduplication enabled, 💰 Price tracking enabled")
    ...
    try:
        ...
        # Step 1: Determine areas to process
        if config['areas']:
            # Use user-specified areas
            session_areas = config['areas']
            logger.info(f"🏙️ Using specified areas: {session_areas}")
        elif is_local_testing and max_properties_limit <= 50:
            # For local testing with small limits, use single area
            ...
            if not is_valid:
                error_msg = f"Environment validation failed: {', '.join(validation_errors)}"
            ...
            if config.get('enable_batching', False):
                ...
                logger.info(f"🔄 Batch processing enabled - Processing areas in batches...")
                ...
                # Process all Tokyo areas
                session_areas = all_tokyo_areas
                logger.info(f"🏙️ Processing all {len(session_areas)} Tokyo areas: {session_areas[:5]}{'...' if len(session_areas) > 5 else ''}")
                
            
            log_structured_message(logger, "INFO", "Area discovery completed", 
                                total_areas=len(all_tokyo_areas) if 'all_tokyo_areas' in locals() else len(session_areas),
        ...
        if not all_urls:
            if dedup_summary:
                ...
                summary_data = {
                    ...
                    "scraper_type": "HTTP_STEALTH_DEDUP",
                    "total_urls_found": dedup_summary.get('total_urls_found', 0),
                    "new_listings": 0,
                    "price_changes": dedup_summary.get('price_changed', 0),
                    "existing_listings": dedup_summary.get('existing_listings', 0),
                    "status": "SUCCESS_NO_NEW_PROPERTIES"
                }
                write_job_summary(summary_data)
                log_structured_message(logger, "INFO", "Scraping completed with no new properties", **summary_data)
                return
            else:
                raise Exception("No listing URLs found")
        
        # Apply property limits if specified
        if config['max_properties'] and config['max_properties'] > 0:
            # Apply user-specified limit
            original_count = len(all_urls)
            all_urls = all_urls[:config['max_properties']]
            logger.info(f"🌐 LIMITED TO {len(all_urls)} PROPERTIES (user specified max_properties={config['max_properties']}, found {original_count} total)")
        else:
            # No limit - process all found properties
            logger.info(f"🚀 PROCESSING ALL {len(all_urls)} PROPERTIES FOUND")
            logger.info(f"🥷 Using stealth timing with human-like delays for {len(all_urls)} properties")
        ...
        if not is_local_testing:
            ...
            log_structured_message(logger, "INFO", "Skipping CloudWatch metrics")
