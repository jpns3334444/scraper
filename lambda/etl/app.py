...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            # Try to use get_config first if available
            bucket = None
            try:
                from util.config import get_config
                config = get_config()
                bucket = config.get_str('OUTPUT_BUCKET')
                logger.info("Using configuration from get_config()")
            except (ImportError, NameError, Exception) as config_error:
                logger.warning(f"get_config unavailable ({config_error}), falling back to environment variables")
                bucket = os.environ.get('OUTPUT_BUCKET')
                if not bucket:
                    raise KeyError("OUTPUT_BUCKET environment variable is required")
                logger.info("Using OUTPUT_BUCKET from environment variables")
            
            if not bucket:
                raise ValueError("OUTPUT_BUCKET is not configured")
...
def process_listings_lean(df: pd.DataFrame, bucket: str, date_str: str) -> Dict[str, Any]:
    ...
    try:
        from util.config import get_config
        config = get_config()
        logger.info("Using configuration from get_config() in process_listings_lean")
    except (ImportError, NameError, Exception) as config_error:
        logger.warning(f"get_config unavailable in process_listings_lean ({config_error}), using fallback config")
...
def calculate_ward_medians(properties: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
    ...
    if wards_needing_fallback:
        try:
            # Try to get configuration with fallback
            config = None
            table_name = None
            try:
                from util.config import get_config
                ...
                logger.warning(f"get_config unavailable for DynamoDB fallback ({config_error}), using environment variable")
